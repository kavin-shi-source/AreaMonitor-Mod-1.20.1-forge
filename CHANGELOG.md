...
### 传送与跨维度
- 修复玩家跨维度传送后，区域保护和物品限制短暂失效的问题
- 修复链式传送到达终点区域后不触发进入提示、游戏模式切换和触发器的问题
- 修复传送目标点安全检测不加载区块的问题，防止玩家传送到下界窒息或主世界掉入虚空

### 时间表与白名单
- 修复时间表自动启用/禁用区域的逻辑反转：手动禁用的区域不再被时间表错误重新启用
- 修复白名单玩家离开区域时仍被触发离开效果（游戏模式切换、标题、触发器）的问题

### 配置与数据安全
- 配置文件保存改为原子写入，防止服务器崩溃时数据丢失
- 添加圆形区域半径上限（2000 格）和矩形区域面积上限，防止误操作导致服务器卡死
- 增强区域名称校验，手动编辑配置文件时拒绝含空格、控制字符等非法字符的名字
- 修复配置文件损坏或缺失时，内存中残留旧数据导致规则不一致的问题

### 客户端兼容
- 修复未安装模组的原版客户端无法连接服务器的问题
- 修复服务端无法正确检测客户端是否安装模组的问题
- GUI 界面命令对原版客户端显示友好提示，不再发送无法处理的网络包

### 界面与输入
- 区域名称输入框长度限制与服务端一致（32 字符）
- 触发器维度格式输入框增加字符过滤
- 命令限制列表自动转小写，避免大小写不匹配
- 修复滚动面板在小幅度滚动时不响应的问题
- 输入框过滤控制字符，防止破坏界面显示
...
### 安全
- GUI 保存触发器时执行完整的数据清理
- 触发器命令过滤扩充：新增 data/function/schedule 等间接命令拦截
- 区域显示名称添加长度上限（64 字符）
